...
#    number of men and women, number of smiles
    ...
    for row in rows:
        row.extend(one_hot_labels(row))
        writer.writerow(row)
    s3.upload_file(Bucket='charityviralreach', Filename='/tmp/rekognitionOutput.csv', key='processing/RekognitionResults/' + key)
